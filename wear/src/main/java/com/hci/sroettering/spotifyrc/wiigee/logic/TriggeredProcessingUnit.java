...
import com.hci.sroettering.spotifyrc.wiigee.event.*;
...
public class TriggeredProcessingUnit extends ProcessingUnit {
	...
	private Vector<GestureTrainedListener> gestureTrainedListeners;
	...
	public TriggeredProcessingUnit() {
        ...
        gestureTrainedListeners = new Vector<>();
	}
	...
	public void handleStartEvent(ActionStartEvent event) {
		...
		if((!this.analyzing && !this.learning) && 
			...
			if(this.trainsequence.size()>0) {
                ...
                this.fireGestureTrainedEvent(this.classifier.getCountOfGestures()-1, this);
		...
		if(this.learning) { // button release and state=learning, stops learning
			...
			if(this.current.getCountOfData()>0) {
				...
				if(recognized!=-1) {
					...
					this.fireGestureEvent(false, -1, 0.0);
					Log.write("######");
    ...
    public void addGestureTrainedListener(GestureTrainedListener listener) {
        gestureTrainedListeners.add(listener);
    }

    private void fireGestureTrainedEvent(int id, TriggeredProcessingUnit source) {
        GestureTrainedEvent event = new GestureTrainedEvent(id, source);
        for(GestureTrainedListener listener: gestureTrainedListeners) {
            listener.gestureTrained(event);
        }
    }

	@Override
	public void loadGesture(String filename) {
		GestureModel g = com.hci.sroettering.spotifyrc.wiigee.util.FileIO.readFromFile(filename);
		this.classifier.addGestureModel(g);
	}

	@Override
	public void saveGesture(int id, String filename) {
		com.hci.sroettering.spotifyrc.wiigee.util.FileIO.writeToFile(this.classifier.getGestureModel(id), filename);
	}
